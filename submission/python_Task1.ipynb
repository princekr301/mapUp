 ...
 "cells": [
   ...
   "outputs": [
    {
     "data": {
      ...
      "text/plain": [
       "          0     1      2     3    4   5    6      7\n",
       "index  id_1  id_2  route  moto  car  rv  bus  truck\n",
       "0         0     0      0     0    0   0    0      0"
      ]
     },
     "execution_count": 44,
  ...
  },
  {
   ...
   "source": [
    ...
    "    # creating a pivot table for the Data frame\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "        \n",
    "\n",
    "#df[\"Category\"] = df[\"car\"].apply(cat)\n",
    "#dict(sorted(df[\"Category\"].value_counts().to_dict().items()))"
   ]
  },
  {
   "cell_type": "markdown",
  ...
  },
  {
   ...
   "source": [
    ...
    "    condition = [(data[\"car\"]<15),((data[\"car\"]>15) & (data[\"car\"]<=25)),(data[\"car\"]>25)]\n",
    "    status = [\"Low\",\"Medium\",\"High\"]\n",
    "    data[\"car_type\"] = np.select(condition,status,default=\"unknown\")\n",
    "    category = data[\"car_type\"].value_counts().to_dict()\n",
    "    cat = dict(sorted(category.items()))\n",
    "    return cat\n",
    "    "
   ]
  },
  ...
  },
  {
   ...
   "outputs": [],
   "source": [
    "def get_bus_indexes(data_source):\n",
    "    # read the dataset\n",
    "    df = pd.read_csv(data_source)\n",
    "    \n",
    "    #caluculating the mean value of the bus\n",
  ...
  },
  {
   ...
   "outputs": [],
   "source": [
    "def filter_routes(source):\n",
    "    df = pd.read_csv(source)\n",
    "    df_route = df.groupby(\"route\")[\"truck\"].mean()\n",
    "    x = df_route[df_route>7].index.tolist()\n",
    "    print(x)"
   ]
  },
  {
  ...
  },
  {
   ...
   "outputs": [],
   "source": [
    "def multiply_matrix(dataset_source):\n",
    "    df = pd.read_csv(dataset_source)\n",
    "    # creating the pivot table \n",
    "    result_df = df.pivot(index='id_1', columns='id_2', values='car').fillna(0)\n",
    "    # applying the condition if Number is greater than 20 then multiply to 0.75 else multiply to 1.25\n",
    "    modified_matrix = result_df.applymap(lambda x:x*0.75 if x>20 else x*1.25)\n",
    "    # round the entire value with 1 decimal\n",
   ...
   "outputs": [
    {
     "data": {
      "text/plain": [
       "array(['Monday', 'Thursday', 'Saturday', 'Tuesday', 'Wednesday'],\n",
       "      dtype=object)"
      ]
     },
     "execution_count": 121,
  ...
  },
  {
   ...
   "source": [
    ...
    "\n",
    "#     # Check if the time range covers a full 24-hour period\n",
    "#     time_range_check = ((df['end_datetime'] - df['start_datetime']).dt.total_seconds() == 24 * 60 * 60)\n",
    "\n",
    "#     # Check if the time range spans all 7 days of the week\n",
    "#     day_of_week_check = (df['start_datetime'].dt.dayofweek.nunique() == 7)\n",
    "\n",
    "#     # Combine the checks\n",
    "#     completeness_check = time_range_check & day_of_week_check\n",
    "\n",
    ...
    "\n",
    "# # Example usage\n",
    "# # Assuming 'dataset_2' is your DataFrame from dataset-2.csv\n",
    "# dataset_2 = pd.DataFrame(...)  # Replace ... with your actual DataFrame\n",
    "\n",
    "# # Apply the function\n",
    "# completeness_result = check_timestamp_completeness(dataset_2)\n",
    "\n",
    "# # Display the result\n",
  ...
  },
  {
   ...
   "outputs": [],
   "source": [
    " #df[\"st_time\"] = pd.to_datetime(df[\"startDay\"] + ' ' + df[\"startTime\"])"
   ]
  },
  {
   "cell_type": "code",
  ...
  },
  {
   ...
   "source": [
    ...
    "\n",
    "# Continue with your analysis or processing...\n"
   ]
  }
 ],
 "metadata": {
